 ...
 */
const subscribableCache = new WeakMap<PromiseLike<unknown>, ProxyPromise<unknown>>()
...
export class Unpromise<T> implements ProxyPromise<T> {
	...
	static resolve<T>(value: T | PromiseLike<T>) {
		...
		return Unpromise.proxy(promise).subscribe() as SubscribedPromise<Awaited<T>>
	}
	...
	static async any<T extends readonly unknown[] | []>(values: T): Promise<Awaited<T[number]>>
	static async any<T>(values: Iterable<T | PromiseLike<T>>): Promise<Awaited<T>> {
	 ...
	 * promises accumulating .then() and .catch() subscribers. */
	static async race<T extends readonly unknown[] | []>(values: T): Promise<Awaited<T[number]>>
	 ...
	 * ```
	 * */
	static async raceReferences<TPromise extends Promise<unknown>>(promises: readonly TPromise[]) {
