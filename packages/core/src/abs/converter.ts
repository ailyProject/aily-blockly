...
import { getGlobalBlockMetas } from './blockMeta'
import { BlocklyAbsParser } from './parser'

import type { AbiToAbsOptions, AbsToAbiResult } from './types'
...
export function convertAbiToAbs(abiJson: any, options: AbiToAbsOptions = {}): string {
	const { includeHeader = true, indentStr = '    ', includeBlockIds = false, explicitBlockTypes = true } = options
...
const runtimeArgsOrderCache = new Map<
	string,
	Array<{ name: string; kind: 'field' | 'valueInput' | 'statementInput' }> | null
>()
 ...
 */
function queryArgsOrderFromBlockly(
	blockType: string
): Array<{ name: string; kind: 'field' | 'valueInput' | 'statementInput' }> | null {
...
function makePortableExtraState(extraState: any): any | null {
	...
	for (const [key, value] of Object.entries(extraState)) {
		...
		if (Array.isArray(value) && value.length > 0 && value.every((v: any) => typeof v === 'string' && v.includes('::')))
			continue
		result[key] = value
	}
 ...
 */
function collectVariableReferences(config: any, variableNameToId: Map<string, string>, variables: any[]): void {
	// 扫描字段中的变量引用
