...
import { getGlobalBlockMetas, loadProjectBlockDefinitions as loadProjectBlockMetas } from './blockMeta'

import type { AbsBlockConfig, AbsBlockMeta, AbsParseResult } from './types'
...
export function loadProjectBlockDefinitions(projectPath: string): void {
	try {
		loadProjectBlockMetas(projectPath)
 ...
 */
function getBlockMeta(blockType: string): Partial<AbsBlockMeta> | undefined {
	// 优先从动态加载的块定义获取
	...
	if (dynamicMetas) {
		...
		if (dynamicMeta) {
			const converted = {
				fieldNames: dynamicMeta.fieldNames.length > 0 ? dynamicMeta.fieldNames : undefined,
				valueInputNames: dynamicMeta.valueInputNames.length > 0 ? dynamicMeta.valueInputNames : undefined,
				statementInputNames: dynamicMeta.statementInputNames.length > 0 ? dynamicMeta.statementInputNames : undefined,
				argsOrder: dynamicMeta.argsOrder && dynamicMeta.argsOrder.length > 0 ? dynamicMeta.argsOrder : undefined,
				hasStatementInput: dynamicMeta.statementInputNames.length > 0,
				isRootBlock: dynamicMeta.isRootBlock,
				isValueBlock:
					dynamicMeta.hasOutput && dynamicMeta.fieldNames.length === 0 && dynamicMeta.valueInputNames.length === 0,
				mutator: dynamicMeta.mutator
			}
...
// Blockly 运行时查询缓存：blockType -> Partial<AbsBlockMeta>
const runtimeBlockMetaCache = new Map<string, Partial<AbsBlockMeta> | null>()
 ...
 * 结果会被缓存以避免重复创建。
 */
function queryBlocklyRuntimeMeta(blockType: string): Partial<AbsBlockMeta> | undefined {
	if (runtimeBlockMetaCache.has(blockType)) {
		return runtimeBlockMetaCache.get(blockType) || undefined
	}
	try {
		const Blockly = (globalThis as { Blockly?: any }).Blockly
		...
		try {
			...
			const result: Partial<AbsBlockMeta> = {
 ...
 */
const FALLBACK_BLOCKS: Record<string, Partial<AbsBlockMeta> & { isValueBlock?: boolean }> = {
	// Arduino 核心块
	arduino_setup: {
		isRootBlock: true,
	...
	math_arithmetic: {
		...
		valueInputNames: ['A', 'B'],
		argsOrder: [
			{ name: 'A', kind: 'valueInput' },
			{ name: 'OP', kind: 'field' },
			{ name: 'B', kind: 'valueInput' }
		]
	},
	math_change: {
		fieldNames: ['VAR'],
		valueInputNames: ['DELTA'],
		argsOrder: [
			{ name: 'VAR', kind: 'field' },
			{ name: 'DELTA', kind: 'valueInput' }
		]
	},
 ...
 * 特殊语法糖映射
 */
const SYNTAX_SUGAR: Record<
	string,
	(args: string[]) => { type: string; fields?: Record<string, any>; inputs?: Record<string, any> }
> = {
...
export class BlocklyAbsParser {
	 ...
	 * @returns 解析结果
	 */
	parse(code: string): AbsParseResult<AbsBlockConfig> {
		this.reset()
	 ...
	 */
	private nodeToBlockConfig(node: AbsNode, position?: { x: number; y: number }): AbsBlockConfig {
		const config: AbsBlockConfig = {
			type: node.type,
			fields: {},
			inputs: {}
		}
		...
		if (node.extraState) {
			config.extraState = node.extraState as AbsBlockConfig['extraState']
		}
		...
		if (!config.extraState) {
			...
			if (extraState) {
				config.extraState = extraState as AbsBlockConfig['extraState']
			}
		}
	...
	private isShadowBlockType(blockType: string): boolean {
		const shadowTypes = new Set(['text', 'math_number', 'logic_boolean', 'variables_get'])
 ...
 */
const CORE_DYNAMIC_BLOCKS: Record<
	string,
	{
		inputPattern: RegExp
 ...
 * 优先使用核心块配置，否则尝试动态检测
 */
function getDynamicBlockConfig(
	blockType: string,
	inputKeys: string[]
): {
	inputPattern: RegExp
 ...
 */
function inferExtraStateFromInputs(blockType: string, inputs: Record<string, any>): Record<string, any> | null {
