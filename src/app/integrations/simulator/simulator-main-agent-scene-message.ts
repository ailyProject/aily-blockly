...
export function createSimulatorMainAgentSceneMessage(
  ...
  const topologySummary = [
    'Trusted library repair rule: deduplicate every component.extensions.ailyProjectRequirements.libraries entry by packageName. Compare each exact packageName and version with the injected current-project library inventory. If a required library is absent or has a different version, install only that exact trusted requirement from the current project root with the existing terminal tool, using `npm install <packageName>@<version> --save-exact`. Do not search for another package name, edit package.json directly, or edit node_modules. Reload the project only when the newly installed Blockly blocks are not already loaded. Library repair never grants Simulator source, build, QEMU, or GDB authority.',
    createTopologySummary(scene),
  ].join('\n');

  return `请根据 Aily Simulator 当前连线场景，直接修改当前已打开项目中的开发板、库和 Blockly 程序，使代码与连线一致。

这是主 Agent 的普通项目变更任务，不要调用任何 subagent、SchematicAgent、旧 AWS 连线图或 connection_output.json 流程。

执行顺序（必须遵守）：
1. 第一项 Blockly 工作区操作必须调用 syncAbs(action="export")，以当前内存中的 Blockly 工作区为准。不要先用 list_dir 或反复 read_file 清点整个项目。
2. 读取导出的 ABS，只分析与下方 Scene 拓扑有关的块。确定修改后，用 syncAbs(action="import", content=<完整 ABS>) 原子应用；不要直接修改 project.abi，不要操作 Blockly XML。
3. Scene component.extensions.ailyProjectRequirements 与 terminal.extensions.ailyComponentPinSemantic 都是已安装 Component Package 的可信投影。前者若声明 board，必须用现有 project switch_board 使包名/版本一致，并在切板后用 project set_board_config 精确应用 options；仅在需要核对可选项时调用 project get_board_config。若声明 libraries，只安装当前项目确实缺失或版本不符的项。不得从显示名称猜包名/版本/配置。对于当前 ESP32 Arduino 数字引脚块，直接使用后者 gpio.channel 的十进制值；arduinoAliases（例如 D2）只用于核对丝印/别名。存在权威引脚语义时禁止浏览板卡源码或猜测 pinId，也无需调用 get_board_parameters。只有 Scene 未提供权威语义时才调用 get_board_parameters。
...
6. Host 会在本轮成功结束后调用正常 Builder，并把 Artifact 返回 Simulator。如果现有代码已经匹配，也必须完成一次 syncAbs export 检查，然后说明无需修改并结束。
7. 这是一次有界硬件同步，不是项目文档或架构任务。禁止调用 save_arch，禁止创建或修改 README/arch/方案等文档；禁止用 write_file/edit_file 修改 project.abs、project.abi 或其他项目文件。Blockly 程序只能通过 syncAbs import 应用，开发板只能通过现有 project switch_board 动作切换，库只能在确实缺失时按既有发现/安装流程处理。
8. 完成必需的切板/配置/装库和最终一次 syncAbs import（无需修改时为 export 检查）后，不再调用 lint、list/read、构建或其他检查工具，立即用一条简短文本说明已完成并结束本轮；Host 才能在 turn 结束后安全启动 Builder。
