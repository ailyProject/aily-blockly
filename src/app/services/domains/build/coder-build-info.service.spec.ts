...
describe('Coder build metadata', () => {
  ...
  it('awaits metadata persistence before acknowledging project-save', async () => {
    const listeners = new Map<string, (action: any) => any>();
    const service = new CodeEditorProProjectService({
      listen: (type: string, callback: (action: any) => any) => listeners.set(type, callback),
    } as any, project, metadata);
    service.registerPersistenceBridge({
      saveAll: async () => {
        files.set(`${root}/sketch/src/helper.h`, 'saved editor contents');
        return { ok: true };
      },
      hasUnsavedChanges: async () => false,
    });
    service.init();
    expect(await listeners.get('project-save')!({ payload: { path: root } })).toEqual({ success: true, path: root });
    expect(readManifest().codeHash).toMatch(/^[a-f0-9]{64}$/);
    expect(readManifest().codeHash).toBe(await metadata.updateCodeHash(root));
  });
