...
class VsixLoader {
    ...
    registerHandlers() {
        ...
        // 读取扩展文件内容
        ipcMain.handle('vsix:read-file', async (event, params) => {
            // console.log('IPC vsix:read-file called with:', params);
            // console.log('typeof params:', typeof params);
            if (!params || typeof params !== 'object') {
                throw new Error('Parameters must be an object');
            }
            const { extensionPath, filePath } = params;
            ...
            try {
                ...
                console.error('Error in vsix:read-file handler:', error);
                throw error;
            }
        });
    ...
    async readExtensionFile(extensionPath, filePath) {
        try {
            console.log('Reading extension file:', extensionPath, filePath);
            ...
            // 规范化文件路径
            let normalizedFilePath = filePath.replace(/^\.\//, ''); // 移除开头的 ./

            // 构建可能的文件路径列表
            const possiblePaths = [
                // // 直接路径
                // path.join(tempExtractDir, normalizedFilePath),
                // // extension 子目录中的路径
                // path.join(tempExtractDir, 'extension', normalizedFilePath),
                // // 如果原路径包含 extension/，移除前缀再试
                // path.join(tempExtractDir, normalizedFilePath.replace(/^extension\//, '')),
                // // 添加常见的文件扩展名
                // path.join(tempExtractDir, normalizedFilePath + '.js'),
                // path.join(tempExtractDir, 'extension', normalizedFilePath + '.js'),
                // // 尝试不同的路径组合
                // path.join(tempExtractDir, 'extension', 'out', path.basename(normalizedFilePath)),
                path.join(tempExtractDir, 'extension', 'out', path.basename(normalizedFilePath) + '.js')
            ];
            ...
            // 如果仍然找不到，列出临时目录的内容以帮助调试
            console.log('File not found. Listing temp directory contents:');
            try {
                const listTempDir = (dir, prefix = '') => {
                    if (fs.existsSync(dir)) {
                        ...
                        for (const item of items) {
                            ...
                            if (stat.isDirectory() && prefix.length < 10) { // 限制递归深度
                                listTempDir(itemPath, prefix + '  ');
                            }
                        }
                    }
                };
                listTempDir(tempExtractDir);
            } catch (listError) {
