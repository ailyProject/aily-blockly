...
1. 在 `store/.uninstalling/<storeKey>.json` 写入临时事务标记。解析器看到标记后立即停止返回 A/B，防止半删除目录重新生效。
2. 停止当前主程序持有的该子应用进程，并检查其他主程序或子进程是否仍持有 B 或版本目录。
3. 删除 B、`store/<storeKey>` 下的 `active.json`、所有版本、staging 和该包专用下载缓存。
4. 删除临时更新缓存中的该子应用目录。
5. 只从 R 根 `package.json`、`package-lock.json` 和属于该包的 `.bin` shim 中移除该包记录；不得修改其他子应用记录或共享依赖。
6. 所有清理成功后删除“卸载中”标记。若 Windows 返回 `EPERM`、`EACCES` 或 `EBUSY`，操作失败并保留标记；下次卸载或重新安装先继续清理，不能留下可启动的残缺版本。
